...
// File contracts/Caketomb.sol

pragma solidity 0.6.12;


contract Caketomb is ERC20Burnable, Operator {
    ...
    function _getCaketombPrice() internal view returns (uint256 _caketombPrice) {
        try IOracle(caketombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Caketomb: failed to fetch caketomb price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _caketombPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_caketombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setcaketombOracle(address _caketombOracle) public onlyOperatorOrTaxOffice {
        require(_caketombOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints caketomb to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of caketomb to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentcaketombPrice = _getCaketombPrice();
            currentTaxRate = _updateTaxRate(currentcaketombPrice);
            if (currentcaketombPrice < burnThreshold) {
                burnTax = true;
            }
        }
