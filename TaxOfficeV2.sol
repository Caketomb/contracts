...
contract TaxOfficeV2 is Operator {
    ...
    constructor(address _caketomb, address _pair, address _wbnb) public {
    ...
    function _excludeAddressFromTax(address _address) private returns (bool) {
        if (!ITaxable(caketomb).isAddressExcluded(_address)) {
    ...
    function taxRate() external returns (uint256) {
        return ITaxable(caketomb).taxRate();
    }
    ...
    {
        require(amtcaketomb != 0 && amtToken != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);

        IERC20(caketomb).transferFrom(msg.sender, address(this), amtcaketomb);
        IERC20(token).transferFrom(msg.sender, address(this), amtToken);
        _approveTokenIfNeeded(caketomb, uniRouter);
        _approveTokenIfNeeded(token, uniRouter);

        _includeAddressInTax(msg.sender);

        uint256 resultAmtcaketomb;
        uint256 resultAmtToken;
        uint256 liquidity;
        (resultAmtcaketomb, resultAmtToken, liquidity) = IUniswapV2Router(uniRouter).addLiquidity(
            caketomb,
            token,
            amtcaketomb,
            amtToken,
        ...
        if (amtcaketomb.sub(resultAmtcaketomb) > 0) {
            IERC20(caketomb).transfer(msg.sender, amtcaketomb.sub(resultAmtcaketomb));
        }
        if (amtToken.sub(resultAmtToken) > 0) {
            IERC20(token).transfer(msg.sender, amtToken.sub(resultAmtToken));
        }
        return (resultAmtcaketomb, resultAmtToken, liquidity);
    }
    ...
    {
        require(amtcaketomb != 0 && msg.value != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);
        ...
        uint256 liquidity;
        (resultAmtcaketomb, resultAmtEth, liquidity) = IUniswapV2Router(uniRouter).addLiquidityETH{value: msg.value}(
            caketomb,
            amtcaketomb,
            amtcaketombMin,
            amtEthMin,
        ...
        return (resultAmtcaketomb, resultAmtEth, liquidity);
    }
    ...
    function taxFreeTransferFrom(
        ...
        IERC20(caketomb).transferFrom(_sender, _recipient, _amt);
